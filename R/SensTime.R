...
st <- sensTime(pathF, pathM, metname, timezone, iter = 300, param = param, speedTresh = 25, 
              ...
              BuffColony = 0.3, FixInt = 2, Interpolate = T)
  ...
  for (i in 1:length(param)) {
    ...
    for (f in 1:iter) {
      ...
      for (o in 1:length(unique(bird$tripID))) {
        ...
        test$TripLength <- difftime(max(test$datetime),min(test$datetime), units = ("min")) #calculate time elapsed between start and end
      ...
      nbT <- length(unique(bird$birdTrip))
      lall <- alltrips[1:nbT, ]
      lall$birdTrip <- c(unique(bird$birdTrip))
      lall$nbTrips <- 0
      
    } else {lall$nbTrips <- length(unique(lall$birdTrip))}
